...
try {
  ...
  const waiting=await page.evaluate(()=> {
    ...
    return {
      ...
      inter_available:document.fonts.check('12px Inter'),
      segoe_variable_available:document.fonts.check('12px "Segoe UI Variable"'),
      segoe_available:document.fonts.check('12px "Segoe UI"'),
    };
  });
  await page.evaluate(()=>{
  ...
  const evidence={
    ...
    waiting,
    on_clock:onClock,
    delta_px:onClock.bar_height-waiting.bar_height,
  };
