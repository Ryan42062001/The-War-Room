...
## Exact-head integration receipt and external-merge race gate (WR-139 R1 proposed addendum; NOT yet accepted)

These additive process safeguards preserve V3.5's independent audit, Manager-only integration, target advancement, path-aware CI, fail-closed exceptions and genuine post-merge canaries. **Documentation is not GitHub enforcement:** until separately authorized, independently audited and actually verified branch protection/ruleset/permission checks are configured, a user or automation with merge/bypass authority can still merge outside this procedure. No audit or Manager comment can retroactively convert that event into an authorized guarded merge.

### Pre-merge authorization receipt for audit-required targets

1. Before integrating an audit-required Builder, product, test, infrastructure or documentation PR, publish the independently authored Auditor evidence PR on one immutable Auditor head, inspect its actual applicable exact-Auditor-head Governance/CI and report, and separately assess the verdict and unresolved findings. A green Builder run or chat-only verdict is not independent acceptance.
2. **Before the target merge**, Manager publishes a distinct dated visible comment on the exact TARGET PR explicitly accepting a scoped independent `PASS` or `PASS WITH NON-BLOCKING FINDINGS`. This receipt binds target task/PR/branch/exact head and cumulative changed paths, Auditor evidence PR/exact head/verdict, exact-final-target required CI run/job IDs, current canonical-main SHA, unresolved non-blocking findings, intended merge method and the **applicable required post-merge CI class** under V3.5 path-aware CI (including any PREEXISTING approved `force-full-ci` requirement). “Audit ready,” an Auditor PR merge, repository ownership or silent passage of time does not constitute this receipt. FAIL or any unresolved blocking finding prohibits the target merge.
3. Immediately before merging, Manager independently refreshes live canonical main, target PR/branch/head/mergeability, actual cumulative path diff, audit evidence publication and exact-head CI. Check receipt head equals current target head and independently audited head. Any material target movement, overlap-risk main advancement, unexpected changed path, required CI failure/absence, blocking finding or ambiguous mergeability STOPS the merge for new review/refreeze. For non-overlapping intervening main documentation, record bounded compatibility review; never silently transfer a verdict to changed source.
4. After all above are satisfied, Manager performs one **exact-head-guarded** merge of the approved target, records merge SHA/parents/actual landed paths and labels the scope `MERGED / NOT CLOSED` until the **original landed merge SHA's applicable genuine canonical-main push CI** succeeds. Auditor evidence publication is NOT automatic Builder integration. A new passing run on an unrelated SHA is not this gate.

### If an external actor already merged a target without that prior Manager receipt

1. Freeze further **ordinary target/dependent/release merges and closure** in the affected lane. Refresh actual canonical main, target and Auditor PR/head/state, merge timestamps/actors, merge commit/parents/landed paths and source blob, current registry, prior Manager receipt or its absence, audit verdict and exact CI identity. Classify `OUT_OF_ORDER_MERGE / RECONCILIATION_REQUIRED`. Do not misrepresent an already-performed merge as Manager-guarded or backdate permission; a later acceptance of the independent verdict is scoped evidence, not retroactive merge authorization.
2. Determine the **originally applicable test class** from V3.5's actual changed paths, classifier result, risk and any explicitly required `force-full-ci` instruction fixed BEFORE integration. Product, tests, scripts, workflow runtime/CI, build or any other FULL-required target retains its mandatory **exact-original-merge-SHA canonical-main PUSH FULL War Room product CI**. A genuinely `.ai/**`-only, correctly GOVERNANCE_ONLY target without prior force-FULL requirement instead needs genuine canonical-main PUSH Governance SUCCESS at that SAME original merge SHA, not an invented FULL product run. Previously force-FULL documentation retains FULL. A green Governance-only run NEVER substitutes for a FULL-required product canary. A target PR's own CI, an Auditor-only CI, an unrelated newer main run or a running/wrong-SHA event never substitutes for the original required canary. Independently inspect actual relevant job logs and unchanged audit/source/landed scope.
3. Issue a **distinct dated prospective Manager exception disposition** after factual custody and independent audit-verdict review, choosing ONLY one supported branch:
   - **AS-IS candidate:** permitted only if the original exact landed merge SHA's originally applicable canonical-main push CI has ACTUALLY completed SUCCESS, independent audit on exactly landed source is separately accepted, scope/main compatibility and unresolved findings pass. Record the approval-order violation permanently and whether the already-landed scope can remain without rollback. Do not call the earlier merge authorized or guarded.
   - **CORRECTIVE / REVERT route:** if original required canary FAILED, is ABSENT, CANCELLED, wrong-SHA, materially stranded/queued/running without a verifiable successful completion, or source/authority custody is doubtful, **AS-IS is prohibited**. Record original exact SHA and actual FAILED/UNVERIFIED/stalled outcome without alteration, reason and bounded impact. Manager may issue a SEPARATE, prospective narrow corrective or revert task/authorization while keeping affected scope `MERGED / NOT CLOSED / REMEDIATION REQUIRED`, dependent/release holds and the original failure receipt. This corrective authorization does **not** require the original run to become SUCCESS and must never reinterpret a later green run as success at the original SHA. Require fresh independent audit of any corrective material change when applicable, an exact-head guarded correction/revert merge and genuine canonical-main push CI of the **corrected/reverted landed SHA** in its OWN applicable preauthorized test class before separately deciding corrected-state closure. Record relationship between original failed merge and corrected commit; preserving an original FAILED canary does not prohibit safe separately reviewed recovery.
4. Escalate a missing/queued/running canary that cannot yield a timely verifiable conclusion as `CANARY_STALLED_OR_UNVERIFIED` with SHA/run/job identity, known cause and custody evidence. Do not assert an invented timeout or automatic cancellation. Permit ONLY explicitly authorized diagnostic/recovery, corrected integration or revert after documented Manager assessment; no ordinary competing work or release, no fabricated run SUCCESS, no newer SHA substitution. If overlap or authority cannot be resolved, keep the affected scope blocked and seek a separately scoped independent review.
5. After AS-IS success OR after a correctly audited corrective/revert landed SHA's OWN genuinely required push CI succeeds, Manager makes a separate truthful final disposition and atomically reconciles registry, task spec, project state, roadmap and handoff. Preserve original result, out-of-order violation, earlier audit verdict and any inherited findings; do not silently rewrite a frozen historical PR or use a corrected-state canary to label the original merge PASS. Require genuine post-reconciliation canonical-main Governance SUCCESS before routing dependent work. A3/A4/A6, provider/deployment/release gates remain independent.

### Pinned original canary and narrow recovery exception

When a mandatory original merge SHA's canonical-main FULL product canary is queued or running, keep that exact push-event SHA pinned and **do not merge unrelated ordinary Manager/documentation/product PRs into main, close the task or activate dependent work** until its outcome is recorded. Non-overlapping proposal branches/draft PRs may be prepared without merging. This interlock **does not forbid** the separately dated corrective/revert/diagnostic recovery route above after a FAILED/ABSENT/CANCELLED/materially stranded canary or independently verified urgent remediation; such narrowly authorized work retains the original SHA/status and its own required post-correction CI. If external actors advance main regardless, independently compare actual changes, preserve original run/identity, assess overlapping risk and stop any unreviewed ordinary integration. A later green docs-only Governance run can never replace an earlier FULL-required canary.

### Documentary counterexample checklist (review, not automated enforcement)

- Original product merge SHA A: actual required FULL SUCCESS plus exact audited custody → eligible for distinct prospective AS-IS disposition; no claim of prior guarded merge.
- A FULL FAILURE/ABSENT/CANCELLED/stranded → original remains FAILED/UNVERIFIED; AS-IS prohibited; separate prospective corrective/revert authorization permitted; independently reviewed corrected SHA B must pass ITS OWN genuine canonical push CI before bounded corrected-state closure. A remains historically failed or unverified.
- Original genuinely docs-only Governance-classified SHA A, NO prior force-FULL: same-SHA canonical push Governance SUCCESS may satisfy the applicable canary for separate truthful AS-IS review. Do not invent a FULL requirement.
- Product/test/CI/runtime FULL-required SHA A: Governance SUCCESS but FULL FAILURE/ABSENT is NOT sufficient; AS-IS prohibited. Docs-only target with explicit PRE-merge force-FULL is also FULL-required and cannot bypass with Governance.
- A later unrelated main SHA C's passing Governance or FULL run never substitutes for A's originally required result; a legitimate new corrective B has its own separate audited scope, required class and post-merge canary.
