...
Status: ACTIVE DEVELOPMENT — WR-D033 activates WR-121 A2 Strategy-only ECR recommendation/explanation contract; Track B custom ranking paused
Current-state snapshot: 2026-09-20, WR-D033 scopes A2 and assigns sole WR-121 Strategy paper-only contract. Refresh live registry/main/CI. Refresh actual `.ai/shared/ACTIVE_TASKS.json`, canonical main and live PR/CI before routing.
...
| A2 | **Smarter, explainable draft recommendations using the accepted ranking sources.** Show value-vs-need, tier cliffs, VORP/replacement value, positional scarcity, roster build and next-pick survival together. | **WR-121 ASSIGNED — Draft Strategy paper-only source-aware existing-behavior + scenario/explanation contract**, branch `wr-121-ecr-draft-recommendation-explanation-contract` after Manager activation/CI. No product code/weights, actual rankings, player data or provider outreach; Builder and Auditor remain idle. Future implementation requires distinct Manager scope and fresh QA. | Auditable candidate comparisons on matched synthetic scenarios; explain relevant ECR value vs ESPN market timing, roster context and why to pick now vs wait, fallback when data stale, missing or unsupported. Preserve WR-D001 and accepted scoring logic until an independently reviewed change; do not fabricate calibrated confidence or new player projections. |
