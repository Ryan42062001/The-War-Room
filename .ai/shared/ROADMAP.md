...
Status: ACTIVE DEVELOPMENT — WR-117 inventory accepted; WR-118 bounded app-side synthetic replay/reconnect test task assigned; custom ranking paused
Current-state snapshot: 2026-09-19, WR-D028 Manager WR-117 acceptance and WR-118 activation. Refresh actual `.ai/shared/ACTIVE_TASKS.json`, canonical main and live PR/CI before routing.
...
| A1 | **Draft-day reliability + reusable synthetic regression coverage.** Prove already implemented app draft-state and recovery mechanics interact correctly before any broader simulator or new UX. | **WR-118 ASSIGNED — Builder test-only deterministic synthetic app-side replay/reconnect regression**, branch `wr-118-synthetic-espn-replay-reconnect-regression` after Manager integration/main CI. No separate Strategy prerequisite for existing mechanical invariants; future recommendation-policy oracle requires distinct Strategy contract. | One compositional numbered ESPN-like slice, reordered/duplicate/stale/partial replay, existing-authority correction, two isolated saved sessions, reload/synthetic reconnect and full/provisional terminal proof. Add one fixture script + package test registration + Builder evidence/handoff only. No production source/companion change; if failing invariant or harness gap, stop for Manager. Fresh independent Auditor on frozen exact implementation SHA before merge; post-merge full CI canary. Existing isolated fixtures credited, no live ESPN Direct proof. |
...
Only the **live** active-task registry and exact Manager task/branch authorize execution. After this Manager transition and post-merge CI, WR-118 Builder TEST-ONLY mechanical regression is the sole assigned task, not full A1–A6 product implementation. Task and branch become actionable only after exact-head Manager PR Governance and canonical-main post-merge CI are verified, then Manager creates branch at that exact main. Historical assignments and older roadmap claims do not convey write access.
