...
Status: ACTIVE DEVELOPMENT — WR-116 one-pass publisher/rights documentary closure assigned; option C unproven, no source/scoring authority
Current-state snapshot: 2026-09-19 after WR-115 research acceptance and WR-D025 WR-116 Manager activation; refresh against `.ai/shared/ACTIVE_TASKS.json` before routing or acting.
...
| --- | --- | --- | --- |
| 0 | **WR-116 — publisher release-policy and upstream-rights documentary closure** | **ASSIGNED — R&D one-pass public policy/legal-documentation only**, `STANDARD_CHAT_HIGH`, branch `wr-116-release-policy-upstream-rights-closure`. WR-115 PR #331 research-only accepted/merged at `71a3fb88cc890d7b521fa5674edf4f45a51da045`. | Inspect only narrowly relevant new official human-readable publisher version/persistence and actual licensor/independent Auditor entitlement text. Return two specific gap verdicts or UNSENT questions then STOP; no generic resurvey, actual release/asset listing/API, provider contact, data, custody, executable protocol, audit or scoring. Ryan's no-local-raw option C remains unproven and source admission blocked. |
