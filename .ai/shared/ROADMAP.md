...
Only the **live** active-task registry and exact Manager task/branch authorize execution. WR-D044 assigns only WR-129, a paper-only Builder inventory for A5. It becomes actionable only after the Manager activation PR passes exact-head Governance, merges, canonical-main push Governance succeeds, and Manager creates the assigned Builder branch from that exact new main. A4 remains season-gated and unassigned; no production/settings/ranking/provider implementation is authorized by roadmap wording. Historical assignments and older roadmap claims do not convey write access.
