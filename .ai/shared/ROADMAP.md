...
Status: ACTIVE DEVELOPMENT — two-track draft product / conditional custom-ranking research; WR-117 draft-day reliability + simulator gap inventory ASSIGNED once this Manager transition integrates
Current-state snapshot: 2026-09-19, WR-D027 roadmap decision. Refresh actual `.ai/shared/ACTIVE_TASKS.json`, canonical main and live PR/CI before routing.
...
| --- | --- | --- | --- |
| A0 | **Existing-functionality inventory + synthetic scenario gap map.** Reuse what is already in the application and tests rather than duplicate it. | **WR-117 ASSIGNED — Builder READ-ONLY documentation-only**, branch `wr-117-draft-day-reliability-simulator-gap-inventory`; sole active assignment after Manager transition and main CI. | Map current app/recommendation/ESPN fallback/sessions and existing `test:draft-invariants`, `test:persistence-recovery`, `test:recovery-failures`, `test:live-mock-fixtures`, phone/browser suites to reproducible synthetic cases; distinguish verified-by-test, implemented-but-untested, missing, unknown. Deliver a gap-based narrow next task; **no code/test/UI change or live ESPN** under WR-117. |
| A1 | **Draft-day reliability + reusable synthetic draft simulator/regression library.** Finish drafts safely and recover when sync/reconnect fails. | **PROPOSED — contingent on A0 gap evidence.** Builder handles missing mechanics/fixtures only after separate Manager approval; Strategy owns ambiguous decision-policy scenarios; fresh Auditor independently checks production changes. | Deterministic synthetic full/partial draft fixtures for snake order, Taken/Mine manual corrections, duplicate/reordered/partial/stale ESPN Board/Pick History snapshots, reconnect and reload, session isolation/migration, terminal/final report, tier cliffs, early QB/WR/RB runs, high-ECR fallers and missing market data. Verify authoritative draft state, lossless recovery, actionable fallback and no false Direct-mode claim. Do not rebuild existing fixtures merely to rename them. |
| A2 | **Smarter, explainable draft recommendations using the accepted ranking sources.** Show value-vs-need, tier cliffs, VORP/replacement value, positional scarcity, roster build and next-pick survival together. | **PROPOSED — Draft Strategy contract BEFORE Builder recommendation-policy changes;** may be scoped after A0 and in parallel with independent A1 mechanics only where dependencies permit. | Auditable candidate comparisons on matched synthetic scenarios; explain relevant ECR value vs ESPN market timing, roster context and why to pick now vs wait, fallback when data stale, missing or unsupported. Preserve WR-D001 and accepted scoring logic until an independently reviewed change; do not fabricate calibrated confidence or new player projections. |
| A3 | **Draft Command Center — on-the-clock desktop and phone UX.** Reduce time/interaction needed to understand the current choice. | **PROPOSED — first inventory the existing Position Tiers, top decision strip, phone view, recommended-pick details and queue before specifying UI gaps.** | Coherent best-available/tier/roster/next-turn view, short two-to-three-player comparison, queue access, legible provenance/stale-sync warnings, keyboard/manual controls and resilient 390px desktop/mobile state. Demonstrate no hidden recommendation-policy change or duplicated ranking/roster state; fresh independent UX/behavior audit for implementation. |
| A4 | **Next-draft-cycle rankings and source freshness refresh.** Retain a trustworthy up-to-date ECR-based board. | **PROPOSED / NOT ASSIGNED; season-specific admission gate.** Existing bundled 2026 baseline remains the authority in current app. | Separately verify future permitted source terms, official ECR/ESPN availability, PPR/league compatibility, player ID and position reconciliation, as-of/freshness provenance, import/count/hash and fallback, with deliberate Manager acceptance and regression review before any bundled dataset replacement. Never silently fetch new season/player rows, import an unlicensed source, substitute custom model or change ranking policy. |
| A5 | **League-personalized planning and optional advanced settings.** Match decisions to the actual league without sacrificing reliable defaults. | **PROPOSED / NOT ASSIGNED.** Inventory existing 2–20 teams, 5–30 rounds, snake/PPR, starter and session configuration first. | Scoped scoring/roster settings and saved plan, opt-in keeper or risk-preference support only with explicit compatibility/evidence and separate Strategy contracts. Unsupported settings fail visibly rather than silently reuse PPR assumptions; prevent cross-session leakage. Post-draft report improvements are gap-driven because one already exists. |
| A6 | **Formal draft-ready release gate.** Decide when the established assistant is ready for an actual draft. | **PROPOSED / NOT ASSIGNED**, after relevant A1–A5 accepted work; earlier release readiness may happen without all optional A5 features. | Repeatable end-to-end synthetic mock + deliberate manual fallback, authoritative pick/order and session recovery, league-input and ranking freshness/rights, recommendation regressions/provenance, phone/desktop accessibility, companion/app compatibility, security boundaries, full applicable CI, deployment and rollback evidence; explicit Manager go/no-go. Green Governance or synthetic fixtures alone do not prove live ESPN structured Direct. |
...
Only the **live** active-task registry and exact Manager task/branch authorize execution. Immediately after this roadmap transition, WR-117 Builder inventory is the sole assigned task, not A1–A6 production implementation. Task and branch become actionable only after exact-head Manager PR Governance and canonical-main post-merge CI are verified, then Manager creates branch at that exact main. Historical assignments and older roadmap claims do not convey write access.
