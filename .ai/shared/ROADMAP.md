...
Status: ACTIVE DEVELOPMENT — WR-129 A5 league-settings inventory activation; A4 next-cycle ranking refresh season-gated; Track B custom ranking paused
Current-state snapshot: 2026-09-21, WR-D044 defers A4 source refresh until a real next-cycle source is available and assigns WR-129 as the A5 read-only league-settings/personalization inventory after activation gates. No production or provider work is authorized.
...
| A5 | **League-personalized planning and optional advanced settings.** Match decisions to the actual league without sacrificing reliable defaults. | **WR-129 CLOSED / inventory accepted; WR-130 ASSIGNED AFTER WR-D045 ACTIVATION GATES.** WR-130 is limited to repairing the demonstrated existing 5–30 round-contract inconsistency across current app/persistence/external-pick/Companion paths, with fresh independent audit required. Alternate scoring/custom-roster/keeper/risk/default-profile features remain unassigned. | Scoped scoring/roster settings and saved plan, opt-in keeper or risk-preference support only with explicit compatibility/evidence and separate Strategy contracts. Unsupported settings fail visibly rather than silently reuse PPR assumptions; prevent cross-session leakage. Post-draft report improvements are gap-driven because one already exists. |
...
Only the **live** active-task registry and exact Manager task/branch authorize execution. WR-D045 closes WR-129 and assigns only WR-130, the bounded audit-required 5–30 round-contract repair. It becomes actionable only after the Manager activation PR passes exact-head Governance, merges, canonical-main push Governance succeeds, and Manager creates the assigned Builder branch from that exact new main. A4 remains season-gated and unassigned; no alternate scoring/custom-roster/keeper/risk/ranking/provider work is authorized by roadmap wording. Historical assignments and older roadmap claims do not convey write access.
