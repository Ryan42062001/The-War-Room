...
Status: ACTIVE DEVELOPMENT — WR-115 source-specific rights/immutable auditor re-fetch paper gate assigned; no source/scoring authority
Current-state snapshot: 2026-09-19 after WR-D024 provisional owner selection and WR-115 Manager activation; refresh against `.ai/shared/ACTIVE_TASKS.json` before routing or acting.
...
| --- | --- | --- | --- |
| 0 | **WR-115 — source-specific immutable re-fetch rights/audit feasibility** | **ASSIGNED — R&D public-documentation/metadata only**, `STANDARD_CHAT_HIGH`, branch `wr-115-source-specific-immutable-refetch-rights`; WR-D024 accepted provisional owner option C + full reference-level audit requirement. | Test official public asset/license/provenance/immutable-version preservation and lawful independent auditor re-fetch preconditions for future F27/T27/F28/T28 source roles, and minimal lawful derived evidence. Exactly two paper reports + R&D handoff. No actual source/release byte inspection, provider contact, custody, protocol, audit, scoring or production ranking. Fail closed if option C cannot be supported; never silently switch to A/B. |
| 1 | **WR-113 / WR-112 / WR-111 / WR-110 — prior prospective research** | **CLOSED — research accepted only**; WR-113 PR #325 exact head `af35bf0b59f733c7c31473a6a24006467e445834` integrated as `d352a92f254bb7247892be7a32b939781792af50`. | `RIGHTS_UNVERIFIED / INSUFFICIENT_NEW_CONFIRMATION_DATA`; no new future source or protocol, and no automatic 2029 season. |
...
WR-115 is the **sole assigned task in the active-only registry** once Manager activation merges and post-merge CI passes. No historical worker branch or non-assigned future task conveys authority. Proposed work above is planning, not permission to run tests on the user's ESPN account, alter data/production, create downstream worker assignments, or expand infrastructure. Manager must refresh the registry and live PR/CI state before any transition; WR-110, WR-111, WR-112 and WR-113 are closed after research-only integration. Previous task branches grant no further write or scoring authority. The external ESPN Direct Work investigation concluded without repository writes. WR-D018 records the bounded feasibility disposition; it grants no new task, browser permission, account access, or implementation authority.
