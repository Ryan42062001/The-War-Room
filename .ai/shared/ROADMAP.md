...
Status: ACTIVE DEVELOPMENT — WR-D076 closes blocked WR-149 and assigns ONE-PASS WR-150 hosted-runner isolation feasibility; no recovery PASS/deployment/live ESPN/A6/release
Current-state snapshot: 2026-09-23 WR-149 BLOCKED_BROWSER_OR_RUNNER accepted, merged 7cfda9affebe7ba0c6324b4ea5a600cf3f3b2f43, main Governance #35906916148 SUCCESS. No A→B→A began. WR-150 investigates ONE concrete existing-browser-runner + network-isolation preflight candidate or defers the local surrogate; conditional 2027 release target unchanged.
...
| R1 — Bounded nonproduction recovery evidence | **WR-149 CLOSED BLOCKED_BROWSER_OR_RUNNER; WR-150 ONE-PASS hosted-runner isolation feasibility ASSIGNED after WR-D076 gates.** Historical hosted FULL CI showed Chromium on ubuntu-latest, not enforced proxy-safe/browser+service-worker loopback-only isolation. WR-150 must find exactly one minimal credible separately authorized/audited runner security pilot OR return NO_QUALIFIED_RUNNER and defer the local surrogate. No app, local A→B→A or independent recovery evidence was executed under WR-149. No experiment or runner security-probe code is authorized by WR-150. | WR-147 public production rollback REHEARSAL_NOT_SAFE remains. Nonproduction local success, if separately achieved and audited, would only show local bytes/browser/cache behavior, NOT Pages build/deploy/CDN/prod rollback. If no safe runner, preserve the limitation explicitly for formal A6 rather than endlessly retry. |
