...
2. The compact market line and expanded "Market timing basis" use the existing `getMarketTimingDetails` attribution. Known market shows labeled ESPN board, ESPN ADP, combined board+ADP, or FantasyPros ADP fallback and qualitative "heuristic" timing; no `N% survival`. Unknown market shows "Market timing unknown — no survival estimate" and "Source: Unknown market". Engine's internal unknown-market sentinel remains 50 with nonadjacent next-turn context; the display does **not** misrepresent it as an observed 50% chance. A per-player freshness caveat is printed, not a fabricated live/player-specific update time. FantasyPros ECR still supplies VALUE, separate from ESPN/FP fallback timing.
...
| ESPN board / ADP | `N% survival` with no established calibration or source caveat | "Market timing · ESPN [source] (heuristic)"; expanded source, market rank inputs and per-player freshness-unverified caveat |
| Missing ESPN and FantasyPros ADP | Internal neutral 50 displayed as `50% survival`, suggesting evidence of 50% odds | "Market timing unknown — no survival estimate"; "Source: Unknown market" in expanded details; numeric internal 50 unchanged |
| FantasyPros ADP fallback | Timing could appear ESPN-derived | "Market timing · FantasyPros ADP fallback (heuristic)"; explicitly labeled fallback, no invented ESPN timestamp |
